...
const MLD_Filters = {
    ...
    initSearchAndFilters: function() {
        ...
        // This function sets up the keyup listener for a search input
        const setupSearchListener = (inputId, suggestionsId) => {
            $(inputId).on('keyup', e => {
                ...
                MLD_Map_App.debounceTimer = setTimeout(() => {
                    ...
                    if (term.length >= 2) {
                        // Pass the suggestions container ID to the API layer
        ...
        // Set up listeners for both search inputs
        ...
        $('#bme-property-type-select').on('change', function(e, isProgrammatic) {
            ...
            // Only reset other filters if this was a manual change by the user.
            // A programmatic change happens on initial load to set the type from the URL or localStorage.
            if (!isProgrammatic) {
                MLD_Map_App.modalFilters = MLD_Filters.getModalDefaults(); // Reset modal filters
                MLD_Filters.restoreModalUIToState(); // Update UI to reflect reset
                MLD_Core.updateUrlHash();
                MLD_API.refreshMapListings(true); // Refresh map with new type and reset filters
            }
            
            // These should always run to update UI components based on the (newly set) property type.
            MLD_Core.updateModalVisibility();
            MLD_API.fetchDynamicFilterOptions(); // This will fetch options like 'home type' based on the new property type
            MLD_Filters.renderFilterTags(); // Re-render tags to show the new property type and any other active filters
        });
        
        $(document).on('click', e => {
            if (!$(e.target).closest('#bme-search-bar-wrapper, #bme-search-bar-wrapper-modal').length) {
                $('#bme-autocomplete-suggestions').hide();
                $('#bme-autocomplete-suggestions-modal').hide();
            }
        });
     ...
     * @param {Array} suggestions - The suggestion data from the API.
     * @param {string} suggestionsId - The ID of the container to render into.
     */
    ...
    addKeywordFilter: function(type, value) {
        ...
        // Clear both inputs and hide both suggestions
    ...
    renderFilterTags: function() {
        ...
        // Removed the placeholder changing logic here

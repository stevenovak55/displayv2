...
const MLD_API = {
    ...
    refreshMapListings: function(forceRefresh = false) {
        ...
        const isInitial = app.isInitialLoad; // Capture initial load state
        ...
        if (!forceRefresh) {
            ...
            // Add a check here for zoom level crossing the threshold (17) for cluster markers
        ...
        let requestData = { 
            ...
            is_initial_load: isInitial // Pass the initial load flag
        };
        
        // Do not send map bounds on initial load to allow for radius search
        ...
        .done(function(response) {
            if (response.success && response.data) {
                ...
                // On initial load or new filter, fit map to the results
                if (forceRefresh && (response.data || []).length > 0) {
                    MLD_Core.fitMapToBounds(response.data);
                }
                
                // Trigger background cache process
     ...
     * @param {string} suggestionsId - The ID of the container to render suggestions into.
     */

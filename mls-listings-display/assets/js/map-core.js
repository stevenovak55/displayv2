...
const MLD_Map_App = {
        ...
        if (bmeMapData.provider === 'google') {
            try {
                ...
                this.map.addListener('idle', () => {
                    // Determine if this is the very first data fetch that needs to apply filters from URL or do the default radius search.
                    const isFirstFetch = this.isInitialLoad || this.hasUrlFilters;
                    MLD_API.refreshMapListings(isFirstFetch);

                    // After this first fetch, both flags should be false to allow normal map interaction.
                    if (isFirstFetch) {
                        this.isInitialLoad = false;
                        this.hasUrlFilters = false;
                    }
                });
	...
	postInitSetup: function() {
		...
		// This is the critical step: check the URL for filters immediately.
		// restoreStateFromUrl will set MLD_Map_App.selectedPropertyType if it's in the hash.
		this.hasUrlFilters = MLD_Core.restoreStateFromUrl();
		
		// If filters were found in the URL, we should not do the default "initial load" (radius search).
		if (this.hasUrlFilters) {
			this.isInitialLoad = false;
		} else {
			// If no filters in URL, try loading property type from localStorage
		...
		// Programmatically set the property type selector and trigger the change handler.
		// The 'true' flag indicates this is not a manual user change, preventing filter resets.
		jQuery('#bme-property-type-select').val(this.selectedPropertyType).trigger('change', [ true ]);
	
		// The rest of the setup is now handled by the 'change' event listener.
	}
};
...
const MLD_Core = {
     ...
     * @returns {boolean} - True if filters were found and applied, false otherwise.
	 */
		...
		for (const [key, value] of params.entries()) {
			...
			if (key === 'PropertyType') {
                ...
                if(typeof defaultFilters[key] === 'boolean') {
                    newModalFilters[key] = value === 'true';
    ...
    /**
     * Updates the listing count indicator message on the map.
     * @param {number} showing - The number of listings currently visible.
     * @param {number} total - The total number of listings matching the filters.
     */
    updateListingCountIndicator: function(showing, total) {
        const $indicator = jQuery('#bme-listings-count-indicator');
        if (total > 0) {
            $indicator.text(`Showing ${showing} of ${total} Listings`).show();
        } else {
            $indicator.hide();
        }
    },
...
/**
 * NEW: A general utilities object to hold helper functions.
 */
const MLD_Utils = {
    /**
     * Converts a field key (like 'WaterfrontYN') into a human-readable label ('Waterfront').
     * @param {string} key The field key.
     * @returns {string} The formatted label.
     */
    get_field_label: function(key) {
        if (bmeMapData && bmeMapData.field_labels && bmeMapData.field_labels[key]) {
            return bmeMapData.field_labels[key];
        }
        if (typeof key !== 'string') return '';
        // Fallback for any labels not passed from PHP
        return key.replace(/YN$|_FLAG$|Enabled$/, '')
                  .replace(/([A-Z])/g, ' $1')
                  .replace(/^MLSPIN /, '')
                  .replace(/^Has/, '')
                  .trim();

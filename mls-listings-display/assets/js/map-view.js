/**
 * MLS Listings Display Map View v5.5.0
 * - FEAT: Replaced toggle with a dropdown for listing mode ("For Sale"/"For Rent").
 * - FEAT: Listing mode selection is now saved to localStorage and restored on subsequent visits.
 * - FEAT: Filter options in the modal now dynamically update based on the sale/rent mode.
 * - FEAT: Adds an expandable "View Details" section to listing popups.
 */
(function($) {
    $(document).ready(function() {
        ...
        async function initMap() {
            ...
            loadInitialMode(); // Load from localStorage first
            restoreStateFromUrl(); // Override with URL hash if present
            initDynamicFilters(); // Fetch filters based on final mode
        }
        ...
        function loadInitialMode() {
            ...
            if (savedMode === 'For Rent') { // Only need to check for non-default
        ...
        function initSearchAndFilters() {
            ...
            $('#bme-listing-mode-select').on('change', function() {
                ...
                localStorage.setItem('bmeListingMode', listingMode); // Save selection

                // Reset modal filters and refresh everything
        ...
        function initEventDelegation() {
            $('body').on('click', '.bme-view-details-btn', function(e) {
                ...
                const $detailsContainer = $card.find('.bme-card-full-details');
                ...
                if ($detailsContainer.is(':visible')) {
                    ...
                    if ($detailsContainer.data('loaded')) {
                        ...
                        fetchAndShowDetails(listingId, $card, $button);
                    }
                }
            });
        }
        ...
        function initDynamicFilters() {
            // Pass the current listing mode to the backend
        ...
        function populateHomeTypes(subtypes) {
            ...
            container.html(html);
            // After populating, ensure the UI reflects the current filter state
            restoreModalUIToState();
        }
        ...
        function restoreStateFromUrl() {
            ...
            for (const [key, value] of params.entries()) {
                ...
                if (key === 'listingMode') {
                    listingMode = value;
                    $('#bme-listing-mode-select').val(listingMode);
                    ...
                    else if (getModalDefaults().hasOwnProperty(key)) {
        ...
        function getCombinedFilters(currentModalState = modalFilters) {
            ...
            // Always add the PropertyType based on the main toggle
            finalFilters.PropertyType = listingMode === 'For Sale' ? ['Residential'] : ['Residential Lease'];
            return finalFilters;
        }
        ...
        function updateUrlHash() {
            ...
            // Add listingMode to URL if it's not the default
            if (listingMode !== 'For Sale') {
                params.set('listingMode', listingMode);
            }

            for (const key in combined) {
                // Don't add PropertyType to hash, it's derived from listingMode
        ...
        function createCardHTML(listing) {
            ...
            return `<div class="bme-listing-card" data-listing-id="${listing.ListingId}">
                        ...
                        <div class="bme-card-details">
                            ...
                            <button class="bme-view-details-btn">View Details</button>
                        </div>
                        <div class="bme-card-full-details"></div>
                    </div>`;
        }

        function fetchAndShowDetails(listingId, $card, $button) {
            ...
            $.post(bmeMapData.ajax_url, {
                ...
                if (response.success) {
                    ...
                    const $detailsContainer = $card.find('.bme-card-full-details');
                    $detailsContainer.html(formatFullDetails(details)).data('loaded', true);
                    $detailsContainer.slideDown();
        ...
        function formatFullDetails(details) {
            ...
            const fieldMap = {
                'Public Remarks': details.PublicRemarks,
                'Property Subtype': details.PropertySubType,
                'Architectural Style': details.ArchitecturalStyle,
                'Year Built': details.YearBuilt,
                'Lot Size': `${details.LotSizeAcres} Acres`,
                'County': details.CountyOrParish,
                'MLS Area Major': details.MLSAreaMajor,
                'MLS Area Minor': details.MLSAreaMinor,
                'Structure Type': details.StructureType,
                'Construction Materials': details.ConstructionMaterials,
                'Parking': `${details.ParkingTotal} spaces (${details.ParkingFeatures})`,
                'Waterfront': details.WaterfrontYN == 1 ? 'Yes' : 'No',
                'Pool': details.PoolFeatures,
                'Heating': details.Heating,
                'Cooling': details.Cooling,
                'Association Fee': `$${details.AssociationFee} / ${details.AssociationFeeFrequency}`,
                'Taxes': `$${details.TaxAnnualAmount} (${details.TaxYear})`,
                'List Agent ID': details.ListAgentMlsId,
                'List Office ID': details.ListOfficeMlsId
            };

            for (const label in fieldMap) {
                const value = fieldMap[label];
                if (value && value.trim() !== '' && value.trim() !== '0' && !value.includes('null')) {

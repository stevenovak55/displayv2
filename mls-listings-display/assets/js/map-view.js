/**
 * MLS Listings Display Map View v11.5.0
 * - FIX: Corrected a bug where pins would not reappear after exiting multi-unit "focus mode". The exit function now correctly triggers a full re-render of the markers in the current viewport.
 * - FIX: Resolved an issue where map pins would not update on pan events. The map state detection logic has been made more robust to ensure refreshes are triggered correctly after both panning and zooming.
 * - FIX: Corrected the marker display logic to ensure dots properly convert to price pins. The logic now accurately assesses the number of unique locations in the current viewport and displays up to 75 price/cluster pins, only using dots for overflow in dense areas.
 * - PERF: Adjusted background cache fetching to use smaller batches (200) with a longer delay (1.5s) to reduce server load.
 * - FEAT: Increased the maximum number of visible price pins from 25 to 75.
 */
(function($) {
    $(document).ready(function() {
        ...
        const BATCH_SIZE = 200; // Number of listings to fetch per background request
        ...
        function fetchAllListingsInBatches(page = 1, filters = getCombinedFilters()) {
            if (isFetchingCache && page === 1) return; // A fetch is already in progress
            if (page === 1) {
                console.log("Starting background cache refresh...");
                isFetchingCache = true;
                allListingsCache = { data: [], timestamp: null, total: 0 }; // Reset cache
            }
        ...
        const refreshMapListings = (forceRefresh = false) => {
            ...
            if (!forceRefresh) {
                ...
                if (!centerChanged && !zoomChanged) {
                    return; // Exit if map state hasn't changed
                }
            }
        ...
        function updateMarkersOnMap(listingsInView) {
            ...
            // 2. Add new markers that are not already on the map
            ...
            // 3. Update the global markers array
        ...
        function enterUnitFocusView(group, focusedMarkerId) {
            ...
            // Add back only the focused marker
            if (focusedMarker) {
                if (bmeMapData.provider === 'google') {
                ...
                markers.push(focusedMarker); // Repopulate markers array with just this one
            }
        ...
        function updatePriceSliderRangeAndHistogram() {
            ...
            // Also toggle for the outlier bar if it exists

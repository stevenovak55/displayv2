/**
 * MLS Listings Display Map View v7.1.3
 * - FIX: Completely rewrote the Open House date parsing logic to correctly handle full ISO date-time strings from the API, ensuring timezone conversions are accurate and the tag is reliably generated. This is the definitive fix for the visibility issue.
 */
(function($) {
    $(document).ready(function() {
        ...
        function getModalDefaults() {
            return { price_min: '', price_max: '', beds_min: 0, beds_max: 0, baths_min: 0, baths_max: 0, home_type: [], status: ['Active'], sqft_min: '', sqft_max: '', year_built_min: '', year_built_max: '', lot_size_min: '', lot_size_max: '', waterfront_only: false, open_house_only: false };
        }
        ...
        function restoreStateFromUrl() { const hash = window.location.hash.substring(1); if (!hash) return; const params = new URLSearchParams(hash); const newKeywordFilters = {}; const newModalFilters = getModalDefaults(); for (const [key, value] of params.entries()) { const values = value.split(','); if (key === 'listingMode') { listingMode = value; $('#bme-listing-mode-select').val(listingMode); } else if (['City', 'Building Name', 'MLS Area Major', 'MLS Area Minor', 'Postal Code', 'Street Name', 'MLS Number', 'Address'].includes(key)) { newKeywordFilters[key] = new Set(values); } else { if (['home_type', 'status'].includes(key)) newModalFilters[key] = values; else if (key === 'waterfront_only' || key === 'open_house_only') newModalFilters[key] = value === 'true'; else if (getModalDefaults().hasOwnProperty(key)) newModalFilters[key] = value; } } keywordFilters = newKeywordFilters; modalFilters = newModalFilters; renderFilterTags(); restoreModalUIToState(); }
            ...
            if (openHouseData && Array.isArray(openHouseData) && openHouseData.length > 0) {
                ...
                const upcoming = openHouseData
                    .map(oh => {
                        ...
                        // The API provides a full ISO 8601 string. Ensure it's parsed as UTC.
        ...
        function formatQuickDetails(details) {
            ...
            const fieldMap = {
                ...
                'Pool': formatDetailValue(details.PoolFeatures), 'Heating': formatDetailValue(details.Heating), 'Cooling': formatDetailValue(details.Cooling),

/**
 * MLS Listings Display Map View v10.2.0
 * - FEAT: Overhauled the price filter to use a dynamic, dual-handle range slider with a histogram bar chart.
 * - FEAT: Added a new AJAX call to fetch price distribution data based on the current context of other filters.
 * - FEAT: Implemented custom, dependency-free JavaScript for rendering the histogram and handling the slider mechanics.
 * - FEAT: Synchronized the slider, histogram, and manual input fields for a seamless user experience.
 * - FIX: Prevented the filter modal from closing accidentally when a price slider handle is dragged and released outside the modal area.
 * - FIX: The "See Listings" button now updates its count dynamically when the price slider is adjusted.
 * - FIX: The slider now reacts to manual input, and its range expands if the entered price is outside the current bounds.
 * - FIX: Manual price inputs are now correctly formatted as currency on blur.
 */
(function($) {
    $(document).ready(function() {
        ...
        let priceSliderData = { min: 0, max: 0, distribution: [] };
        ...
        function initSearchAndFilters() {
            ...
            $filtersModal.on('click', e => {
                // Only close if the click is on the overlay itself and not during a drag operation
        ...
        function initPriceSlider() {
            ...
            function startDrag(e) {
                ...
                $('#bme-filters-modal-overlay').addClass('is-dragging'); // Prevent modal close on drag
            ...
            function stopDrag() {
                activeHandle = null;
                // Use a small timeout to ensure the mouseup event has fully propagated
                // before re-enabling the modal's close functionality.
                setTimeout(() => {
        ...
        function updatePriceFromSlider() {
            ...
            let minInputVal = parseFloat($('#bme-filter-price-min').data('raw-value'));
            let maxInputVal = parseFloat($('#bme-filter-price-max').data('raw-value'));
            if (isNaN(minInputVal)) minInputVal = Infinity;
            if (isNaN(maxInputVal)) maxInputVal = -Infinity;

            const displayMin = Math.min(minInputVal, priceSliderData.min);
            const displayMax = Math.max(maxInputVal, priceSliderData.max);
            const displayRange = displayMax - displayMin;

            const currentMin = (displayRange > 0) ? Math.round(displayMin + (minPercent / 100) * displayRange) : displayMin;
            const currentMax = (displayRange > 0) ? Math.round(displayMin + (maxPercent / 100) * displayRange) : displayMax;
        ...
        function updateSliderFromInput() {
            ...
            if (isNaN(minVal) && isNaN(maxVal)) {
                 // Both are empty, reset to full range
            ...
            if (isNaN(minVal)) minVal = priceSliderData.min;
            if (isNaN(maxVal)) maxVal = priceSliderData.max;
            
            if (minVal > maxVal) {
                [minVal, maxVal] = [maxVal, minVal];
                $('#bme-filter-price-min').val(formatCurrency(minVal)).data('raw-value', minVal);
                $('#bme-filter-price-max').val(formatCurrency(maxVal)).data('raw-value', maxVal);
            }

            const displayMin = Math.min(minVal, priceSliderData.min);
            const displayMax = Math.max(maxVal, priceSliderData.max);
            const displayRange = displayMax - displayMin;
            ...
            if (displayRange > 0) {
                minPercent = ((minVal - displayMin) / displayRange) * 100;
                maxPercent = ((maxVal - displayMin) / displayRange) * 100;
            }

            document.getElementById('bme-price-slider-handle-min').style.left = Math.max(0, Math.min(100, minPercent)) + '%';
            document.getElementById('bme-price-slider-handle-max').style.left = Math.max(0, Math.min(100, maxPercent)) + '%';
        ...
        function updatePriceSliderRangeAndHistogram() {
            ...
            $('#bme-price-histogram .bme-histogram-bar').each(function(index) {
                const barPercent = (index / priceSliderData.distribution.length) * 100;
                $(this).toggleClass('in-range', barPercent >= minPercent && barPercent < maxPercent);
            });
        }

        function updatePriceSliderUI() {
            const { min, max, distribution } = priceSliderData;
            
            const currentMin = modalFilters.price_min !== '' ? modalFilters.price_min : min;
            const currentMax = modalFilters.price_max !== '' ? modalFilters.price_max : max;
            ...
            if (!distribution || distribution.length === 0 || max === 0) {

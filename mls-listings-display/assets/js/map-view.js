/**
 * MLS Listings Display Map View v10.7.0
 * - FEAT: Redesigned "Beds" filter to be a multi-select and "Baths" to be a minimum-value select for a more intuitive user experience.
 * - FEAT: Price histogram now includes a special "outlier" bar for listings priced above the slider's max, with a descriptive tooltip on hover.
 * - FEAT: Setting the max price slider handle to 100% now correctly signifies "or more" by clearing the max price filter.
 * - FEAT: Price slider now uses a 95th percentile calculation on the backend to create a more focused, user-friendly range that ignores extreme outliers.
 * - FEAT: Added a help note to the UI to clarify that manual price inputs can exceed the slider's visual range.
 * - PERF: Unit cluster markers are now conditionally displayed based on zoom level and total marker count to reduce map clutter.
 */
(function($) {
    $(document).ready(function() {
        ...
        function groupAndDisplayMarkers(listings) {
            ...
            for (const key in listingsByLocation) {
                ...
                if (group.length > 1) {
                    // This is a cluster. Apply the new logic.
                    if (totalListings > MAX_PINS && currentZoom < CLUSTER_ZOOM_THRESHOLD) {
                        // Too many listings and zoomed out, show a simple dot instead of the cluster marker.
                        createDotMarker(group[0], parseFloat(lng), parseFloat(lat));
                    } else {
                        // Show the full cluster marker.

/**
 * MLS Listings Display Map View v9.5.4
 * - FIX: Implemented a check in `handleMapIdle` to prevent unnecessary marker refreshes. The script now tracks the last map center and zoom level and only redraws markers if the map has moved significantly or if a filter has been applied. This resolves the core flickering issue for all marker types.
 * - FIX: Removed inline styles from `createDotMarker` that were preventing the CSS hover effect from working correctly.
 */
(function($) {
    $(document).ready(function() {
        ...
        // --- NEW: State for preventing unnecessary refreshes ---
        ...
        const refreshMapListings = (forceRefresh = false) => {
            ...
            // FIX: Check if the map has moved significantly before refreshing
            if (!forceRefresh && lastMapCenter && lastMapZoom) {
                ...
                if (!centerChanged && !zoomChanged) {
                    return; // Abort refresh if map hasn't moved enough
                }
            }
            ...
            $.post(bmeMapData.ajax_url, requestData, function(response) {
                if (response.success) {
                    // Update last known position only after a successful refresh

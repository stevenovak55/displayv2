/**
 * MLS Listings Display Map View v6.1.0
 * - FEAT: "Quick Details" modal now has a "View Full Details" button linking to the single property page.
 * - FIX: Data that is stored as a JSON array (e.g., ["Hot Water","Oil"]) is now formatted into a clean, comma-separated string.
 * - CHORE: Removed List Agent/Office IDs from the Quick Details view.
 */
(function($) {
    $(document).ready(function() {
        ...
        // --- STATE MANAGEMENT ---
        let listingMode = 'For Sale'; // Default mode
        let keywordFilters = {}; 
        ...
        function populateHomeTypes(subtypes) {
            ...
            if (!subtypes || subtypes.length === 0) { container.html('<p>No specific home types available for this category.</p>'); return; }
        ...
        function getModalDefaults() {
            return { price_min: '', price_max: '', beds_min: 0, beds_max: 0, baths_min: 0, baths_max: 0, home_type: [], status: ['Active'], sqft_min: '', sqft_max: '', year_built_min: '', year_built_max: '', lot_size_min: '', lot_size_max: '', waterfront_only: false };
        }
        ...
        function restoreModalUIToState() {
            ...
            $('#bme-filter-sqft-min').val(modalFilters.sqft_min); $('#bme-filter-sqft-max').val(modalFilters.sqft_max); $('#bme-filter-year-built-min').val(modalFilters.year_built_min); $('#bme-filter-year-built-max').val(modalFilters.year_built_max); $('#bme-filter-lot-size-min').val(modalFilters.lot_size_min); $('#bme-filter-lot-size-max').val(modalFilters.lot_size_max);
            $('#bme-filter-amenities input[value="WaterfrontYN"]').prop('checked', modalFilters.waterfront_only);
        }
        function restoreStateFromUrl() { const hash = window.location.hash.substring(1); if (!hash) return; const params = new URLSearchParams(hash); const newKeywordFilters = {}; const newModalFilters = getModalDefaults(); for (const [key, value] of params.entries()) { const values = value.split(','); if (key === 'listingMode') { listingMode = value; $('#bme-listing-mode-select').val(listingMode); } else if (['City', 'Building Name', 'MLS Area Major', 'MLS Area Minor', 'Postal Code', 'Street Name', 'MLS Number', 'Address'].includes(key)) { newKeywordFilters[key] = new Set(values); } else { if (['home_type', 'status'].includes(key)) newModalFilters[key] = values; else if (key === 'waterfront_only') newModalFilters[key] = value === 'true'; else if (getModalDefaults().hasOwnProperty(key)) newModalFilters[key] = value; } } keywordFilters = newKeywordFilters; modalFilters = newModalFilters; renderFilterTags(); restoreModalUIToState(); }
        function getCombinedFilters(currentModalState = modalFilters) { const combined = {}; for (const type in keywordFilters) { if (keywordFilters[type].size > 0) combined[type] = Array.from(keywordFilters[type]); } const tempCombined = { ...combined, ...currentModalState }; const finalFilters = {}; for (const key in tempCombined) { const value = tempCombined[key]; const defaultValue = getModalDefaults()[key]; if (JSON.stringify(value) !== JSON.stringify(defaultValue)) { if ((Array.isArray(value) && value.length > 0) || (!Array.isArray(value) && value && value != 0)) { finalFilters[key] = value; } } } finalFilters.PropertyType = listingMode === 'For Sale' ? ['Residential'] : ['Residential Lease']; return finalFilters; }
        function updateUrlHash() { const params = new URLSearchParams(); const combined = getCombinedFilters(); if (listingMode !== 'For Sale') params.set('listingMode', listingMode); for (const key in combined) { if (key === 'PropertyType') continue; const value = combined[key]; if (Array.isArray(value) || value instanceof Set) { if (Array.from(value).length > 0) params.set(key, Array.from(value).join(',')); } else if (value) { params.set(key, value.toString()); } } const newHash = '#' + params.toString(); if (window.location.hash !== newHash) history.replaceState(null, '', newHash); }
        ...
        /**
         * Cleans up values that might be JSON arrays stored as strings.
         */
        ...
        function formatQuickDetails(details) {
            ...
            const fieldMap = {
                ...
                'County': details.CountyOrParish, 'Structure Type': details.StructureType,
                'Parking': details.ParkingFeatures, 'Waterfront': details.WaterfrontYN == 1 ? 'Yes' : 'No',
                'Pool': details.PoolFeatures, 'Heating': details.Heating, 'Cooling': details.Cooling,
            ...
            for (const label in fieldMap) {
                const rawValue = fieldMap[label];
                const formattedValue = formatDetailValue(rawValue);
                if (formattedValue && formattedValue !== 'N/A') {
                    html += `<div class="bme-detail-item"><strong>${label}:</strong> <span>${formattedValue}</span></div>`;
                }
            }
            html += '</div>';
            // Add the new "View Full Details" button at the end

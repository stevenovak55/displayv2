 ...
 * v3.3.0
 * - FIX: Corrected a fatal error in the `mld_format_display_value` helper function by reordering the logic to handle arrays before other data types. This resolves the 500 server error and improves data cleaning.
 * - FIX: Updated the `mld_render_grid_item` helper to intelligently format labels for boolean fields ending in 'YN' (e.g., `PoolPrivateYN` becomes "Has Pool Private").
 * - FIX: Replaced `PoolYN` with the correct `PoolPrivateYN` field in the amenities section.
 */
...
function mld_format_display_value($value, $na_string = 'N/A') {
    // First, attempt to decode if it's a JSON string that represents an array or object
    if (is_string($value) && (strpos(trim($value), '[') === 0 || strpos(trim($value), '{') === 0)) {
        $decoded = json_decode($value, true);
        if (json_last_error() === JSON_ERROR_NONE) {
            $value = $decoded; // Use the decoded value for further processing
        }
    }

    // Handle arrays first, as they are a distinct data type
    if (is_array($value)) {
    ...
    // Handle numeric booleans (like 1 or 0)
    if (is_numeric($value)) {
        if ($value == 1) return 'Yes';
        if ($value == 0) return 'No';
    }
    
    // Handle string booleans (like "Yes" or "No")
    if (is_string($value)) {
    ...
    // For any other value, return it as a cleaned-up string
...
function mld_render_grid_item($label, $value) {
    // Intelligently format labels for fields ending in 'YN' (e.g., PoolPrivateYN becomes "Has Pool Private")
    $pretty_label = $label;
...
$is_rental = $listing['PropertyType'] === 'Residential Lease';
...
$is_price_drop = (float)$listing['OriginalListPrice'] > (float)$listing['ListPrice'];

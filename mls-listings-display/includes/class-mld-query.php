...
class MLD_Query {
    ...
    public static function get_distinct_filter_options( $filters = [] ) {
        ...
        foreach ($fields_to_fetch as $field => $exclude_keys) {
            ...
            $field_where_clause = $where_clause ? $where_clause . ' AND ' : ' WHERE ';
            $field_where_clause .= "`{$field}` IS NOT NULL AND `{$field}` != ''";
    ...
    public static function get_listings_for_map( $north, $south, $east, $west, $filters = null, $is_new_filter = false, $count_only = false, $is_initial_load = false ) {
        ...
        // The distance calculation can be part of the SELECT clause for sorting and filtering.
        ...
        if ($count_only) {
            $select_clause = "SELECT COUNT(id)";
        } else if ($is_initial_load) {
            $select_clause = "SELECT {$select_fields}, {$distance_calc} AS distance";
        } else {
        ...
        // For initial load, we use a radius-based WHERE clause.
        if ($is_initial_load) {
            $where_conditions[] = $wpdb->prepare("({$distance_calc}) < %f", $radius_miles);
        } 
        // Otherwise, use bounds if they are valid and it's not a new filter search.
        else if ( ! $is_new_filter && !$count_only && $bounds_are_valid ) {
    ...
    public static function get_autocomplete_suggestions( $term ) {
        ...
        $fields_to_search = [
            ...
            'PostalCode' => 'Postal Code', // Corrected key

 ...
 *
 * v3.3.0
 * - FEAT: Adds WHERE clause to handle the new `available_by` date filter for rentals.
 */
class MLD_BME_Query {
    ...
    public static function get_listings_for_map( $north, $south, $east, $west, $filters = null, $is_new_filter = false, $count_only = false ) {
        ...
        if ( $has_filters ) {
            ...
            if ( ! empty( $filters['PropertyType'] ) ) $filter_where_group[] = $wpdb->prepare( "PropertyType IN (" . implode( ', ', array_fill(0, count($filters['PropertyType']), '%s') ) . ")", $filters['PropertyType'] );
            ...
            if ( ! empty( $filters['keywords'] ) ) $filter_where_group[] = $wpdb->prepare( "PublicRemarks LIKE %s", '%' . $wpdb->esc_like( $filters['keywords'] ) . '%' );
            if ( ! empty( $filters['stories'] ) ) {
                if ( $filters['stories'] === '3+' ) {
                    $filter_where_group[] = $wpdb->prepare( "StoriesTotal >= %d", 3 );
                } else {
                    $filter_where_group[] = $wpdb->prepare( "StoriesTotal = %d", intval( $filters['stories'] ) );
                }
            }
            if ( ! empty( $filters['waterfront_only'] ) && $filters['waterfront_only'] ) $filter_where_group[] = "WaterfrontYN = 1";
            if ( ! empty( $filters['pool_only'] ) && $filters['pool_only'] ) $filter_where_group[] = "PoolYN = 1";
            if ( ! empty( $filters['garage_only'] ) && $filters['garage_only'] ) $filter_where_group[] = "GarageYN = 1";
            if ( ! empty( $filters['fireplace_only'] ) && $filters['fireplace_only'] ) $filter_where_group[] = "FireplaceYN = 1";
            if ( ! empty( $filters['open_house_only'] ) && $filters['open_house_only'] ) $filter_where_group[] = "OpenHouseData IS NOT NULL AND OpenHouseData != '[]' AND OpenHouseData != ''";

            // New Rental Availability Filter
            if ( ! empty( $filters['available_by'] ) ) {
                $date = $filters['available_by'];
                $filter_where_group[] = $wpdb->prepare( "(MLSPIN_AvailableNow = 1 OR (AvailabilityDate IS NOT NULL AND AvailabilityDate <= %s))", $date );
            }

 ...
 *
 * v4.4.0
 * - FEAT: Added the `get_all_listings_for_cache` method to support the new client-side background caching feature. It fetches all listings with filtering and pagination, but without map bounds.
 * - FEAT: `get_price_distribution` now provides an `outlier_count` for listings priced above the 95th percentile, allowing the UI to render a more useful histogram that isn't skewed by extreme high-end properties.
 * - FEAT: The main distribution calculation is now focused on the range up to the 95th percentile for a better user experience.
 * - PERF: Removed highly inefficient `ORDER BY RAND()` from `get_listings_for_map`.
 * - SEC: Added stricter type casting and validation within the `build_filter_conditions` method.
 */
class MLD_BME_Query {
    ...
    public static function get_price_distribution( $filters = [] ) {
        ...
        // Get all prices for the given context to calculate percentiles
        ...
        // Calculate the 95th percentile to find a reasonable display max
        ...
        // If display_max ends up being 0 or less than min, default to the actual max
        if ($display_max_price <= $min_price && $price_count > 0) {
            $display_max_price = (float) end($prices);
        }
    
        // Calculate histogram based on the new display range
        $num_buckets = 20;
        $bucket_size = ( $display_max_price - $min_price ) / $num_buckets;
        if ( $bucket_size <= 0 ) $bucket_size = 1;
    
        // Filter the query for the histogram to only include prices within the display range
        ...
        // Get the count of outliers above the display_max
    ...
    private static function build_filter_conditions($filters, $exclude_keys = []) {
        ...
        if (!in_array('beds_min', $exclude_keys) && !empty($filters['beds_min'])) {
            $beds_min = intval($filters['beds_min']);
            $beds_max = !empty($filters['beds_max']) ? intval($filters['beds_max']) : 0;
            if ($beds_max > 0 && $beds_max >= $beds_min) {
                $conditions[] = $wpdb->prepare("BedroomsTotal BETWEEN %d AND %d", $beds_min, $beds_max);
            } else {
                $conditions[] = $wpdb->prepare("BedroomsTotal >= %d", $beds_min);
            }
        }
        
        if (!in_array('baths_min', $exclude_keys) && !empty($filters['baths_min'])) {
            $bath_calc = "(BathroomsFull + (BathroomsHalf * 0.5))";
            $baths_min = floatval($filters['baths_min']);
            $baths_max = !empty($filters['baths_max']) ? floatval($filters['baths_max']) : 0;
            if ($baths_max > 0 && $baths_max >= $baths_min) {
                $conditions[] = $wpdb->prepare("{$bath_calc} BETWEEN %f AND %f", $baths_min, $baths_max);
            } else {
                $conditions[] = $wpdb->prepare("{$bath_calc} >= %f", $baths_min);
            }
        }

        if (!in_array('home_type', $exclude_keys) && !empty($filters['home_type']) && is_array($filters['home_type'])) {
            $placeholders = implode(', ', array_fill(0, count($filters['home_type']), '%s'));
            $conditions[] = $wpdb->prepare("PropertySubType IN ({$placeholders})", $filters['home_type']);
        }
        if (!in_array('status', $exclude_keys) && !empty($filters['status']) && is_array($filters['status'])) {
            $placeholders = implode(', ', array_fill(0, count($filters['status']), '%s'));
            $conditions[] = $wpdb->prepare("StandardStatus IN ({$placeholders})", $filters['status']);
        }
    ...
    public static function get_listings_for_map( $north, $south, $east, $west, $filters = null, $is_new_filter = false, $count_only = false ) {
        ...
        if (empty($where_conditions)) {
            // A sensible default if no filters and no map bounds are provided
        ...
        // PERF: Replaced ORDER BY RAND() with a more performant query.
        // For an initial load without filters, we can order by last modification time or simply limit the results.
        if ( ! $is_new_filter && empty($filters) && !$count_only) {
            // Fetch most recently modified listings first. Assumes a 'ModificationTimestamp' column exists.
            // If not, a simple LIMIT is better than RAND().
            if ($wpdb->get_var("SHOW COLUMNS FROM `{$table_name}` LIKE 'ModificationTimestamp'")) {
                 $sql .= " ORDER BY ModificationTimestamp DESC LIMIT 325";
            } else {
                 $sql .= " LIMIT 325";
            }
        } else if (!$count_only) {
            // Limit the maximum number of listings returned to prevent overwhelming the browser.
            $sql .= " LIMIT 1000";
        }
    ...
    public static function get_autocomplete_suggestions( $term ) {
        ...
        foreach ( $fields_to_search as $field_name => $type_label ) {
            $sql_parts[] = $wpdb->prepare(
                "(SELECT DISTINCT %s AS type, `$field_name` AS value FROM `$table_name` WHERE `$field_name` LIKE %s AND `$field_name` IS NOT NULL AND `$field_name` != '')",
                $type_label,
                $term_like
            );
        }

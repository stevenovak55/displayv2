 ...
 *
 * v2.2.0
 * - FEAT: Added `get_distinct_filter_options` to dynamically fetch values for the new filters modal.
 * - FEAT: Added `count_only` mode to `get_listings_for_map` for instant filter count updates.
 * - FEAT: `get_distinct_filter_options` now fetches `PropertyType` for the new "Listing Type" filter.
 * - FIX: The main query now correctly calculates total bathrooms from `BathroomsFull` and `BathroomsHalf`.
 * - FIX: The main query now supports range selections for beds and baths (`min`/`max`).
 * - REFACTOR: `get_listings_for_map` now uses the `$is_new_filter` flag to determine the query type.
 */
class MLD_BME_Query {

    public static function get_distinct_filter_options() {
        global $wpdb;
    ...
    public static function get_listings_for_map( $north, $south, $east, $west, $filters = null, $is_new_filter = false, $count_only = false ) {
        ...
        if ( $has_filters ) {
            ...
            // Handle new modal filters
            if ( ! empty( $filters['listing_type'] ) ) {
                $filter_where_group[] = $wpdb->prepare( "PropertyType IN (" . implode( ', ', array_fill(0, count($filters['listing_type']), '%s') ) . ")", $filters['listing_type'] );
            }
            ...
            if ( ! empty( $filters['waterfront_only'] ) && $filters['waterfront_only'] === 'true' ) $filter_where_group[] = "WaterfrontYN = 1";
        ...
        // If there are no filters, default to Active status (unless we're just counting).
        else if (!$count_only) {
        ...
        if ( ! $is_new_filter && ! $has_filters && !$count_only) {
            ...
            $sql .= " LIMIT 1000"; // Add a reasonable limit to broad searches
        }

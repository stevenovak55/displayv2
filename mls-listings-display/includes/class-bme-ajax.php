 ...
 *
 * v3.4.0
 * - FEAT: Added `get_all_listings_for_cache_callback` to support the new client-side background caching feature. This allows the frontend to fetch all listings in batches.
 * - SEC: Added explicit sanitization and type casting for all incoming POST data.
 * - STABILITY: Improved error handling and responses for invalid requests.
 ...
 * - FEAT: Added `get_price_distribution_callback` to fetch data for the new price slider histogram.
 */
class MLD_BME_AJAX {
    ...
    public function get_all_listings_for_cache_callback() {
        ...
        try {
            $result = MLD_BME_Query::get_all_listings_for_cache($filters, $page, $limit);
    ...
    public function get_price_distribution_callback() {
        ...
        try {
            $distribution = MLD_BME_Query::get_price_distribution( $filters );
    ...
    public function get_map_listings_callback() {
        ...
        if ( isset( $_POST['filters'] ) && ! empty( $_POST['filters'] ) ) {
            ...
            if ( json_last_error() === JSON_ERROR_NONE && is_array( $decoded_filters ) ) {
                // Here you could add an extra layer of sanitization if needed, looping through the array.
                // For now, we trust the structure and sanitize within build_filter_conditions.

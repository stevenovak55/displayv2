 ...
 *
 * v1.9.0
 * - FEAT: `get_filter_options_callback` now accepts a `listing_mode` to provide context-aware filter options.
 * - FEAT: Adds `get_listing_details_callback` to fetch all data for a single property.
 * - FEAT: Adds `get_live_listing_details_callback` to fetch expanded, real-time data from the Bridge API.
 */
class MLD_BME_AJAX {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_get_live_listing_details', [ $this, 'get_live_listing_details_callback' ] );
        add_action( 'wp_ajax_nopriv_get_live_listing_details', [ $this, 'get_live_listing_details_callback' ] );
    }

    /**
     * Fetches live, expanded data for a single listing from the Bridge API.
     */
    public function get_live_listing_details_callback() {
        ...
        try {
            $details = MLD_BME_Query::get_live_details_from_api( $listing_id );
            if ( $details ) {
                wp_send_json_success( $details );
            } else {
                wp_send_json_error( 'Could not fetch live details for this listing.' );
            }
        } catch ( Exception $e ) {
            wp_send_json_error( 'An API error occurred: ' . $e->getMessage() );
        }
        wp_die();
    }
    ...
    public function get_filter_options_callback() {
        check_ajax_referer( 'bme_map_nonce', 'security' );
        $listing_mode = isset($_POST['listing_mode']) ? sanitize_text_field($_POST['listing_mode']) : 'For Sale';
        try {
            $options = MLD_BME_Query::get_distinct_filter_options($listing_mode);

 ...
 *
 * v4.0.0
 * - FEAT: Overhauled the Beds & Baths filter UI to be more intuitive. "Beds" is now a multi-select, and "Baths" is a minimum-value selection.
 * - FEAT: Added a help text note below the price filter inputs to clarify that they can be used for values outside the slider's range.
 */
class MLD_BME_Shortcodes {
    ...
    public function enqueue_map_assets() {
        ...
        if ( is_a( $post, 'WP_Post' ) && ( has_shortcode( $post->post_content, 'bme_listings_map_view' ) || has_shortcode( $post->post_content, 'bme_listings_half_map_view' ) ) ) {
            ...
            wp_enqueue_style( 'mld-main-css', MLD_PLUGIN_URL . 'assets/css/main.css', [], '7.3.0' );
            wp_enqueue_script( 'mld-map-view-js', MLD_PLUGIN_URL . 'assets/js/map-view.js', [ 'jquery' ], '10.7.0', true );

 ...
 *
 * v3.0.0 
 * - FEAT: Adds an "Open House" checkbox to the filters modal, allowing users to find properties with upcoming open houses.
 */
class MLD_BME_Shortcodes {
    ...
    public function enqueue_map_assets() {
        ...
        if ( is_a( $post, 'WP_Post' ) && ( has_shortcode( $post->post_content, 'bme_listings_map_view' ) || has_shortcode( $post->post_content, 'bme_listings_half_map_view' ) ) ) {
            ...
            wp_enqueue_style( 'mld-main-css', MLD_PLUGIN_URL . 'assets/css/main.css', [], '3.0.0' );
            wp_enqueue_script( 'mld-map-view-js', MLD_PLUGIN_URL . 'assets/js/map-view.js', [ 'jquery' ], '3.0.0', true );

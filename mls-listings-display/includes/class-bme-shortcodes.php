 ...
 *
 * v3.6.0
 * - FEAT: Passes the `mld_subtype_customizations` to the frontend JavaScript so the map can use custom icons and labels.
 */
class MLD_BME_Shortcodes {
    ...
    public function enqueue_map_assets() {
        ...
        if ( is_a( $post, 'WP_Post' ) && ( has_shortcode( $post->post_content, 'bme_listings_map_view' ) || has_shortcode( $post->post_content, 'bme_listings_half_map_view' ) ) ) {
            ...
            wp_enqueue_script( 'mld-map-view-js', MLD_PLUGIN_URL . 'assets/js/map-view.js', [ 'jquery' ], '9.9.0', true );

...
class BME_Database_Manager {
    ...
    /**
     * Create additional performance indexes, checking if they exist first.
     */
    private function create_indexes() {
        $indexes_to_create = [
            'idx_listings_filter' => [
                'table' => $this->tables['listings'],
                'sql' => "CREATE INDEX idx_listings_filter ON {$this->tables['listings']} (standard_status, property_type, list_price)"
            ],
            'idx_listings_search' => [
                'table' => $this->tables['listings'],
                'sql' => "CREATE INDEX idx_listings_search ON {$this->tables['listings']} (property_type, bedrooms_total, bathrooms_total_integer)"
            ],
            'idx_location_search' => [
                'table' => $this->tables['listing_location'],
                'sql' => "CREATE INDEX idx_location_search ON {$this->tables['listing_location']} (city, state_or_province, postal_code)"
            ],
        ];
        
        foreach ($indexes_to_create as $index_name => $index_info) {
            if (!$this->index_exists($index_info['table'], $index_name)) {
                $this->wpdb->query($index_info['sql']);
            } else {
                error_log("BME Database Manager: Index '{$index_name}' already exists on '{$index_info['table']}'. Skipping creation.");
            }
        }
    }

    /**
     * Check if a specific index exists on a given table.
     *
    ...
    private function index_exists($table_name, $index_name) {
        $sql = $this->wpdb->prepare(
            ...
            esc_sql($table_name), // esc_sql for table name in backticks
            $index_name
        );

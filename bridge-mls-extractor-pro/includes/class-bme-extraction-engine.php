 ...
 * Version: 1.1 (With Cache Invalidation)
 */
class BME_Extraction_Engine {
    ...
    public function run_extraction($extraction_id, $is_resync = false) {
        ...
        try {
            ...
            // *** FIXED: Invalidate cache on successful run to refresh browser data ***
    ...
    private function process_listings_batch($extraction_id, $batch_listings, &$metrics) {
        try {
            ...
            $agent_ids = [];
            $office_ids = [];
            $listing_keys_active = []; // Only for listings that might have open houses

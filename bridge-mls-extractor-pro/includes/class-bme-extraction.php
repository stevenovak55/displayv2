 ...
 *
 * v3.6.0
 * - FIX: Rewrote the `process_listings` method to build a raw `REPLACE` query. This ensures that the `Coordinates` field (a `POINT` data type) is correctly formatted and inserted in the same atomic operation as the rest of the listing data, resolving the "Cannot get geometry object" database error.
 * v3.5.0
...
class BME_Extraction {
    ...
    private function process_listings($post_id, $listings, $agents_map, $offices_map, $open_houses_map) {
        ...
        foreach ($listings as $listing) {
            ...
            $placeholders_sql = implode(', ', array_fill(0, count($data), '%s'));
            $values = array_values($data);

            $lat = isset($listing['Latitude']) && is_numeric($listing['Latitude']) ? (float) $listing['Latitude'] : null;
            $lon = isset($listing['Longitude']) && is_numeric($listing['Longitude']) ? (float) $listing['Longitude'] : null;

            $sql = "REPLACE INTO `$table_name` ($columns_sql, `Coordinates`) VALUES ($placeholders_sql, ";

            if ($lat !== null && $lon !== null) {
                // Inject the function call directly. This is safe as we've validated lat/lon are numeric.
                $sql .= "ST_PointFromText('POINT($lon $lat)'))";
            } else {
                // Provide a default point if coordinates are missing, because the column is NOT NULL.
                $sql .= "ST_PointFromText('POINT(0 0)'))";
            }

            $prepared_sql = $wpdb->prepare($sql, $values);
            $wpdb->query($prepared_sql);
            // --- END NEW INSERTION LOGIC ---

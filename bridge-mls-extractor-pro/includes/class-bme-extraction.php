...
class BME_Extraction {
        ...
        try {
            ...
            do {
                ...
                if (!empty($data['value'])) {
                    ...
                    $processing_result = $this->process_listings($post_id, $batch_listings, $agents_map, $offices_map, $open_houses_map);
                    
                    $all_processed_info = array_merge($all_processed_info, $processing_result['processed_info']);
                    $all_db_errors = array_merge($all_db_errors, $processing_result['db_errors']);
            ...
            if (!empty($all_db_errors)) {
                $status = 'Completed with errors';
                $message = sprintf('%s completed, but %d listing(s) failed to save to the database. See details for more information.', $run_type, count($all_db_errors));
            } else {
                $status = 'Success';
                $message = sprintf('%s completed. %d listings were added or updated.', $run_type, $total_listings_processed);
            }
            
            $this->log_activity($post_id, $status, $message, $total_listings_processed, $all_processed_info, $all_db_errors);
    ...
    private function process_listings($post_id, $listings, $agents_map, $offices_map, $open_houses_map) {
        ...
        foreach ($listings as $listing) {
            if (empty($listing['ListingKey'])) continue;
            ...
            // Unset the Coordinates column from the main data array to avoid column conflicts,
            // as we will handle it with a specific SQL function. Latitude and Longitude are kept.
            unset($data['Coordinates']);

            // Prepare spatial data
            $lat = $listing['Latitude'] ?? null;
            $lon = $listing['Longitude'] ?? null;
            $point_string = 'POINT(0 0)'; // Default value for data integrity
            if (is_numeric($lat) && is_numeric($lon)) {
                $point_string = sprintf("POINT(%f %f)", (float)$lon, (float)$lat);
            }
            
            // Prepare for INSERT ... ON DUPLICATE KEY UPDATE query
            $columns = array_keys($data);
            $update_clauses = [];
            foreach ($columns as $column) {
                $update_clauses[] = "`$column` = VALUES(`$column`)";
            }

            // Add the Coordinates column to the lists for insertion and update
            $columns[] = 'Coordinates';
            $update_clauses[] = "`Coordinates` = VALUES(`Coordinates`)";

            // Build the SQL query string
            $sql = "INSERT INTO `$table_name` (`" . implode('`, `', $columns) . "`) ";
            $sql .= "VALUES (" . implode(', ', array_fill(0, count($data), '%s')) . ", ST_PointFromText(%s)) ";
            $sql .= "ON DUPLICATE KEY UPDATE " . implode(', ', $update_clauses);
            ...
            if ($result === false) {
                ...
                continue; // Skip to the next listing on failure
            }
        ...
        return ['processed_info' => $processed_info, 'db_errors' => $db_errors];
    }

    private function log_activity($extraction_id, $status, $message, $count, $processed_listings, $db_errors = []) {
        ...
        if ($log_id && !is_wp_error($log_id)) {
            ...
            if (!empty($db_errors)) {
                update_post_meta($log_id, '_bme_log_db_errors', $db_errors);
            }

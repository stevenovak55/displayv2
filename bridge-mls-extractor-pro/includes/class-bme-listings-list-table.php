...
class BME_Advanced_Listings_List_Table extends WP_List_Table {
    ...
    public function prepare_items() {
        // Verify nonce if a filter submission is detected.
        // For GET requests with filters, wp_verify_nonce is often more suitable
        // than check_admin_referer, as check_admin_referer also performs a referer check
        // which can fail with long or complex GET URLs.
        if ( (isset($_REQUEST['filter_standard_status']) ||
              isset($_REQUEST['filter_property_type']) ||
              isset($_REQUEST['filter_city']) ||
              isset($_REQUEST['filter_listing_id']) ||
              isset($_REQUEST['filter_price_min']) ||
              isset($_REQUEST['filter_price_max']) ||
              isset($_REQUEST['filter_bedrooms_min']) ||
              isset($_REQUEST['filter_bathrooms_min']) ||
              isset($_REQUEST['filter_year_built_min']) ||
              isset($_REQUEST['filter_year_built_max']) ||
              isset($_REQUEST['s']) ) && isset($_REQUEST['bme_filter_nonce'])
        ) {
    ...
    private function get_filters_from_request() {
        ...
        // Add the main search query 's' to filters if present
        if (isset($_REQUEST['s']) && !empty($_REQUEST['s'])) {
            $filters['search_query'] = sanitize_text_field($_REQUEST['s']);
        }
            ...
            case 'modification_timestamp':
                ...
                if ($timestamp) {
                    return '<abbr title="' . esc_attr($timestamp) . '">' . 
                           esc_html(human_time_diff(strtotime($timestamp) ?? 0)) . ' ago</abbr>'; // Added null coalesce for strtotime
                }
                return '—';
    ...
    public function get_bulk_actions() {
        // The bulk actions are not directly used for the CSV export button,
        // as the export button is a separate form submission.
        // However, if you wanted to integrate it with the WP_List_Table's
        // native bulk actions, you would define it here.
        // For now, we'll leave it as a placeholder or remove if not needed.
        return []; 
    }

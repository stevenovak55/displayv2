...
class BME_Admin {
    ...
    public function init_hooks() {
        ...
        add_action('admin_notices', [$this, 'display_db_error_notice']);
    }

    /**
     * Displays an admin notice if the database table could not be created on activation.
     */
    public function display_db_error_notice() {
        if (get_transient('bme_db_verification_failed')) {
            $error_message = get_transient('bme_db_creation_error');
            
            echo '<div class="notice notice-error is-dismissible">';
            echo '<p><strong>' . esc_html__('Bridge MLS Extractor Pro: Database Error', 'bridge-mls-extractor') . '</strong></p>';
            echo '<p>' . esc_html__('The required database table could not be created. The plugin may not function correctly.', 'bridge-mls-extractor') . '</p>';
            
            if ($error_message) {
                echo '<p>' . esc_html__('The following error was reported by the database:', 'bridge-mls-extractor') . '</p>';
                echo '<pre style="white-space: pre-wrap; background: #f0f0f1; padding: 10px; border-radius: 4px;">' . esc_html($error_message) . '</pre>';
            }
            
            echo '</div>';

            delete_transient('bme_db_verification_failed');
            delete_transient('bme_db_creation_error');
        }
    }
    ...
    public function render_settings_page() {
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                do_settings_sections('bme-data-management');
                submit_button();
                ?>
            </form>
    ...
    public function register_settings() {
        ...
        // Data Management Setting (for uninstall)
        register_setting(
            'bme_settings_group',
            'bme_delete_on_uninstall',
        ...
        add_settings_section(
            'bme_data_management_section',
            __('Data Management', 'bridge-mls-extractor'),
            null,
            'bme-data-management'
        );
        ...
        add_settings_field(
            'bme_delete_on_uninstall_field',
            __('Plugin Deletion', 'bridge-mls-extractor'),
    ...
    /**
     * Render the checkbox field for the uninstall setting.
     */
    public function render_delete_on_uninstall_field() {
        $option = get_option('bme_delete_on_uninstall');
        ?>
        <label>
            <input type="checkbox" name="bme_delete_on_uninstall" <?php checked($option, 'on'); ?> />
            <?php _e('Delete all listings, logs, and settings when deleting the plugin from the WordPress plugins page.', 'bridge-mls-extractor'); ?>
        </label>
        <p class="description">
            <strong><?php _e('Warning:', 'bridge-mls-extractor'); ?></strong> <?php _e('This action is irreversible. If you plan to reinstall the plugin later, leave this unchecked to preserve your data.', 'bridge-mls-extractor'); ?>
        </p>
        <?php
    }

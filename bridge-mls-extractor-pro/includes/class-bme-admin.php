...
class BME_Admin {
    ...
    public static function save_extraction_meta_static($post_id, $post) { // Made static
        ...
        // Validate and sanitize input data using security helper
        $sanitized_data = BME_Security_Helper::sanitize_extraction_profile($_POST);
        
        // Validate extraction profile data
        $errors = BME_Security_Helper::validate_extraction_profile($sanitized_data);
        if (!empty($errors)) {
            foreach ($errors as $error) {
                add_settings_error('bme_pro_settings', 'validation_error', $error, 'error');
            }
            set_transient('settings_errors', get_settings_errors(), 30);
            return;
        }

        // Check for status group conflicts
        $active_group = ['Active', 'Active Under Contract', 'Pending'];
        $archived_group = ['Closed', 'Expired', 'Withdrawn', 'Canceled'];
        
        if (!empty(array_intersect($sanitized_data['statuses'], $active_group)) && 
            !empty(array_intersect($sanitized_data['statuses'], $archived_group))) {
            add_settings_error('bme_pro_settings', 'status_conflict', 
                __('Error: You cannot select statuses from both Active and Archived groups in the same extraction.', 'bridge-mls-extractor-pro'), 'error');
            set_transient('settings_errors', get_settings_errors(), 30);
            return;
        }

        // Save validated data
        update_post_meta($post_id, '_bme_statuses', $sanitized_data['statuses']);
        update_post_meta($post_id, '_bme_lookback_months', $sanitized_data['lookback_months']);
        update_post_meta($post_id, '_bme_cities', $sanitized_data['cities']);
        update_post_meta($post_id, '_bme_states', explode(',', $sanitized_data['states']));
        
        // Save schedule settings
        if (isset($_POST['bme_schedule'])) {
            $schedule = sanitize_key($_POST['bme_schedule']);
            update_post_meta($post_id, '_bme_schedule', $schedule);
        }
        
        // Save agent IDs with additional validation
        if (isset($_POST['bme_list_agent_id'])) {
            $list_agent_id = preg_replace('/[^A-Za-z0-9\-_]/', '', $_POST['bme_list_agent_id']);
            update_post_meta($post_id, '_bme_list_agent_id', $list_agent_id);
        }
        
        if (isset($_POST['bme_buyer_agent_id'])) {
            $buyer_agent_id = preg_replace('/[^A-Za-z0-9\-_]/', '', $_POST['bme_buyer_agent_id']);
            update_post_meta($post_id, '_bme_buyer_agent_id', $buyer_agent_id);
        }
        
        // Log profile update
        BME_Security_Logger::log('extraction_profile_updated', [
            'post_id' => $post_id,
            'statuses' => $sanitized_data['statuses']
        ]);
    ...
    private function handle_admin_action($action_name, $nonce_action, $success_message, $fail_message, $is_resync = false) {
        ...
        if (!$post_id || !current_user_can('edit_post', $post_id) || !check_admin_referer($nonce_action . '_' . $post_id)) {
            ...
            BME_Security_Logger::log('unauthorized_admin_action', [
                'action' => $action_name,
                'post_id' => $post_id
            ]);
            wp_die('Invalid request.');
    ...
    public function sanitize_api_credentials($input) {
        // Store credentials encrypted instead of plain text
        $credential_manager = new BME_Credential_Manager();
        
        $token = sanitize_text_field($input['server_token'] ?? '');
        ...
        // Return dummy data - actual credentials are stored encrypted
        return ['stored' => true];
    }
    ...
    public function render_settings_page() {
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                // Get encrypted credentials for display
                $credential_manager = new BME_Credential_Manager();
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            <input type="password" id="bme_server_token" name="bme_pro_api_credentials[server_token]" value="" class="regular-text" placeholder="<?php echo !empty($encrypted_creds['token']) ? __('Token is securely stored', 'bridge-mls-extractor-pro') : __('Enter API token', 'bridge-mls-extractor-pro'); ?>">
                            <?php if (!empty($encrypted_creds['token'])): ?>
                                <p class="description"><?php _e('API token is encrypted and stored securely. Enter a new value to update.', 'bridge-mls-extractor-pro'); ?></p>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <input type="url" id="bme_endpoint_url" name="bme_pro_api_credentials[endpoint_url]" value="<?php echo esc_attr($encrypted_creds['endpoint'] ?? ''); ?>" class="large-text">
                        </td>
                    </tr>
                </table>

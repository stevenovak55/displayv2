 ...
 * Version: 2.3.4 (Added Live Search)
 */
class BME_Admin {
    ...
    private function init_hooks() {
        ...
        // Corrected the hook registration below
        add_action('save_post_bme_extraction', [$this, 'save_extraction_meta'], 10, 2);
    ...
    public function render_extraction_stats_meta_box($post) {
        ...
        <div class="bme-actions">
            ...
            <a href="<?php echo esc_url($run_url); ?>" class="button button-primary bme-action-button"><?php _e('Run Now', 'bridge-mls-extractor-pro'); ?></a>
            <a href="<?php echo esc_url($resync_url); ?>" class="button button-secondary bme-action-button bme-confirm-resync"><?php _e('Full Resync', 'bridge-mls-extractor-pro'); ?></a>
     ...
     */
    public function save_extraction_meta($post_id, $post) {
        ...
        $has_active = !empty(array_intersect($statuses, $active_group));
        $has_archived = !empty(array_intersect($statuses, $archived_group));
        ...
        if ($has_archived && $lookback_months <= 0) {
    ...
    private function handle_admin_action($action_name, $nonce_action, $success_message, $fail_message, $is_resync = false) {
        if (!isset($_GET['post_id'])) wp_die('Missing post ID.');
    ...
    public function display_admin_notices() {
        ...
        if (isset($_GET['message'])) {
            $messages = [
                ...
                'data_cleared' => ['success', __('Data cleared successfully.', 'bridge-mls-extractor-pro')],

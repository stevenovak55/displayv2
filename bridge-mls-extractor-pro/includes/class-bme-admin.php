 ...
 * FIXED: Form field name mismatch and improved validation
 */
class BME_Admin {
    ...
    private function init_hooks() {
        ...
        // CSV Export handlers
        add_action('admin_post_bme_export_csv', [$this, 'handle_csv_export']);
        ...
        <table class="form-table bme-config-table">
            ...
            <tr>
                ...
                <td>
                    ...
                    <p class="description"><?php _e('Select at least one status. Required field.', 'bridge-mls-extractor-pro'); ?></p>
                </td>
            </tr>
    ...
    /**
     * FIXED: Save extraction meta data with proper field mapping
     */
    public function save_extraction_meta($post_id) {
        ...
        // FIXED: Correct field mapping between form fields and meta keys
        $fields = [
        ...
        foreach ($fields as $meta_key => $post_key) {
            ...
            // Apply appropriate sanitization and validation
            switch ($meta_key) {
                case '_bme_schedule':
                    $value = sanitize_key($value);
                    // Validate against allowed schedules
                    $allowed_schedules = ['none', 'every_15_minutes', 'every_30_minutes', 'hourly', 'twicedaily', 'daily'];
                ...
                case '_bme_cities':
                    ...
                    // Validate city names don't contain invalid characters
                    if (!empty($value)) {
                        $cities = array_map('trim', explode(',', $value));
                ...
                case '_bme_closed_lookback_months':
                    ...
                    break;
                case '_bme_list_agent_id':
                case '_bme_buyer_agent_id':
                    $value = sanitize_text_field($value);
                    // Validate MLS ID format (alphanumeric)
        ...
        // Save array fields with validation
        $statuses = [];
        if (isset($_POST['bme_statuses']) && is_array($_POST['bme_statuses'])) {
            $valid_statuses = ['Active', 'Active Under Contract', 'Pending', 'Closed', 'Expired', 'Withdrawn', 'Canceled'];
            $statuses = array_intersect(array_map('sanitize_text_field', $_POST['bme_statuses']), $valid_statuses);
        }
        update_post_meta($post_id, '_bme_statuses', $statuses);
        
        $states = [];
        if (isset($_POST['bme_states']) && is_array($_POST['bme_states'])) {
            $valid_states = ['MA', 'NH', 'RI', 'VT', 'CT', 'ME'];
            $states = array_intersect(array_map('sanitize_text_field', $_POST['bme_states']), $valid_states);
        }
        ...
        if (empty($statuses)) {
            add_settings_error(
                'bme_extraction_config',
                'no_statuses',
                __('At least one listing status must be selected.', 'bridge-mls-extractor-pro'),
                'error'
            );
        }
    ...
    public function render_database_browser() {
        ...
        <div class="wrap">
            ...
            <form method="get" id="bme-listings-filter" action="<?php echo admin_url('admin.php'); ?>">
                ...
                <div class="bme-filters-panel">
                    ...
                    <div class="bme-filters-actions">
                        ...
                        <a href="<?php echo admin_url('admin.php?page=bme-database-browser'); ?>" class="button button-secondary">
                            <?php _e('Clear Filters', 'bridge-mls-extractor-pro'); ?>
                        </a>
                        <button type="button" id="bme-advanced-toggle" class="button button-secondary">
            ...
            <form method="get" action="<?php echo admin_url('admin.php'); ?>">
                <input type="hidden" name="page" value="bme-database-browser">
                <?php 
                // Preserve filter parameters for sorting
                $filter_params = ['filter_standard_status', 'filter_property_type', 'filter_city', 'filter_listing_id', 'filter_price_min', 'filter_price_max', 'filter_bedrooms_min', 'filter_bathrooms_min'];
            ...
            <!-- Export Modal -->
            <div id="bme-export-modal" style="display: none;">
                <div class="bme-modal-backdrop"></div>
                <div class="bme-modal-content">
                    <div class="bme-modal-header">
                        <h3><?php _e('Export Listings to CSV', 'bridge-mls-extractor-pro'); ?></h3>
                        <button type="button" class="bme-modal-close" onclick="bmCloseExportModal()">&times;</button>
                    </div>
                    <div class="bme-modal-body">
                        <form method="post" action="<?php echo admin_url('admin-post.php'); ?>" id="bme-export-form">
                            <input type="hidden" name="action" value="bme_export_csv">
                            <?php wp_nonce_field('bme_csv_export'); ?>
                            <input type="hidden" name="export_type" id="export_type" value="">
                            <input type="hidden" name="selected_listings" id="selected_listings" value="">
                            
                            <!-- Export filters as hidden fields -->
                            <?php foreach ($filter_params as $param): ?>
                                <input type="hidden" name="filters[<?php echo esc_attr($param); ?>]" value="<?php echo esc_attr($_GET[$param] ?? ''); ?>">
                            <?php endforeach; ?>
                            
                            <div class="bme-export-options">
                                <h4><?php _e('Export Options', 'bridge-mls-extractor-pro'); ?></h4>
                                
                                <label>
                                    <input type="radio" name="export_scope" value="selected" id="export_selected" checked>
                                    <?php _e('Export Selected Listings', 'bridge-mls-extractor-pro'); ?>
                                    <span id="selected_count">(0 selected)</span>
                                </label>
                                
                                <label>
                                    <input type="radio" name="export_scope" value="filtered">
                                    <?php _e('Export All Filtered Results', 'bridge-mls-extractor-pro'); ?>
                                    <span id="filtered_count"></span>
                                </label>
                                
                                <label>
                                    <input type="radio" name="export_scope" value="all">
                                    <?php _e('Export All Listings (Maximum 50,000 records)', 'bridge-mls-extractor-pro'); ?>
                                </label>
                            </div>
                            
                            <div class="bme-export-info">
                                <p><strong><?php _e('Export includes:', 'bridge-mls-extractor-pro'); ?></strong></p>
                                <ul>
                                    <li><?php _e('Basic listing information (MLS #, price, status, etc.)', 'bridge-mls-extractor-pro'); ?></li>
                                    <li><?php _e('Complete address and location data', 'bridge-mls-extractor-pro'); ?></li>
                                    <li><?php _e('Property details (bedrooms, bathrooms, square footage, etc.)', 'bridge-mls-extractor-pro'); ?></li>
                                    <li><?php _e('Financial information (taxes, HOA fees, etc.)', 'bridge-mls-extractor-pro'); ?></li>
                                    <li><?php _e('Property features and amenities', 'bridge-mls-extractor-pro'); ?></li>
                                    <li><?php _e('Agent and office information', 'bridge-mls-extractor-pro'); ?></li>
                                    <li><?php _e('School district information', 'bridge-mls-extractor-pro'); ?></li>
                                </ul>
                            </div>
                            
                            <div class="bme-modal-actions">
                                <button type="submit" class="button button-primary" id="export_submit">
                                    <?php _e('Download CSV', 'bridge-mls-extractor-pro'); ?>
                                </button>
                                <button type="button" class="button" onclick="bmCloseExportModal()">
                                    <?php _e('Cancel', 'bridge-mls-extractor-pro'); ?>
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            .bme-export-options label:hover {
                background: #f5f5f5;
            }
            
            .bme-export-options input[type="radio"] {
                margin-right: 10px;
            }
        ...
        <script>
        jQuery(document).ready(function($) {
            // Initialize Select2 for filter dropdowns - but not for advanced filters initially
            ...
            $('#bme-advanced-toggle').on('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                
                var $panel = $('#bme-advanced-panel');
                var $button = $(this);
                
                if ($panel.is(':visible')) {
                    $panel.slideUp(300, function() {
                        $button.text('<?php _e('Advanced Filters', 'bridge-mls-extractor-pro'); ?>');
                    });
                } else {
                    $panel.slideDown(300, function() {
                        $button.text('<?php _e('Hide Advanced', 'bridge-mls-extractor-pro'); ?>');
                    });
                }
            ...
            <?php 
            $advanced_params = ['filter_price_min', 'filter_price_max', 'filter_bedrooms_min', 'filter_bathrooms_min'];
            foreach ($advanced_params as $param) {
                if (!empty($_GET[$param])) {
                    echo "hasAdvancedFilters = true;\n";
                    break;
                }
            }
            ?>
            ...
            if (hasAdvancedFilters) {
                $('#bme-advanced-panel').show();
                $('#bme-advanced-toggle').text('<?php _e('Hide Advanced', 'bridge-mls-extractor-pro'); ?>');
            }
            
            // Handle bulk actions for export
            $('#doaction, #doaction2').on('click', function(e) {
                var action = $(this).siblings('select').val();
                if (action === 'export_csv' || action === 'export_all_csv') {
                    e.preventDefault();
                    bmOpenExportModal(action);
                    return false;
                }
            });
            
            // Update export form when radio buttons change
            $('input[name="export_scope"]').on('change', function() {
                $('#export_type').val($(this).val());
                bmUpdateExportCounts();
            });
        ...
        function bmOpenExportModal(action) {
            var selectedCount = jQuery('input[name="listing[]"]:checked').length;
            
            if (action === 'export_csv' && selectedCount === 0) {
                alert('<?php _e('Please select at least one listing to export.', 'bridge-mls-extractor-pro'); ?>');
                return;
            }
            
            // Set default export type
            if (action === 'export_csv') {
                jQuery('#export_selected').prop('checked', true);
                jQuery('#export_type').val('selected');
            } else {
                jQuery('input[name="export_scope"][value="filtered"]').prop('checked', true);
                jQuery('#export_type').val('filtered');
            }
            
            bmUpdateExportCounts();
            jQuery('#bme-export-modal').show();
        }
        
        function bmCloseExportModal() {
            jQuery('#bme-export-modal').hide();
        }
        
        function bmUpdateExportCounts() {
            var selectedCount = jQuery('input[name="listing[]"]:checked').length;
            jQuery('#selected_count').text('(' + selectedCount + ' selected)');
            
            // Get filtered count from pagination info
            var paginationText = jQuery('.displaying-num').text();
            var match = paginationText.match(/([0-9,]+)/);
            var filteredCount = match ? match[1] : '0';
            jQuery('#filtered_count').text('(' + filteredCount + ' results)');
            
            // Update selected listings for form
            var selectedIds = [];
            jQuery('input[name="listing[]"]:checked').each(function() {
                selectedIds.push(jQuery(this).val());
            });
            jQuery('#selected_listings').val(selectedIds.join(','));
        }
        </script>
        <?php
    }
    ...
    /**
     * Render filter options with caching and proper SQL escaping
     */
    ...
    public function handle_csv_export() {
        ...
        if (!wp_verify_nonce($_POST['_wpnonce'], 'bme_csv_export')) {
        ...
        if ($export_type === 'selected') {
            $listing_ids = array_map('absint', (array)$listing_ids);
            $listing_ids = array_filter($listing_ids);
            
            if (empty($listing_ids)) {
                wp_die(__('No listings selected for export.'));
            }
        }
        
        // Generate and download CSV
        $this->generate_csv_download($export_type, $listing_ids, $filters);
    }
    
    /**
     * Generate CSV download
     */
    private function generate_csv_download($type, $listing_ids = [], $filters = []) {
        global $wpdb;
        ...
        $sql = "
            SELECT 
                l.listing_id as 'MLS Number',
                l.listing_key as 'Listing Key',
                l.standard_status as 'Status',
                l.property_type as 'Property Type',
                l.property_sub_type as 'Property Subtype',
                l.list_price as 'List Price',
                l.original_list_price as 'Original List Price',
                l.close_price as 'Close Price',
                l.bedrooms_total as 'Bedrooms',
                l.bathrooms_total_integer as 'Bathrooms',
                l.living_area as 'Living Area (Sq Ft)',
                l.year_built as 'Year Built',
                
                -- Address
                ll.unparsed_address as 'Full Address',
                ll.street_number as 'Street Number',
                ll.street_name as 'Street Name',
                ll.city as 'City',
                ll.state_or_province as 'State',
                ll.postal_code as 'Zip Code',
                ll.county_or_parish as 'County',
                ll.latitude as 'Latitude',
                ll.longitude as 'Longitude',
                ll.subdivision_name as 'Subdivision',
                
                -- School Information
                ll.elementary_school as 'Elementary School',
                ll.middle_or_junior_school as 'Middle School',
                ll.high_school as 'High School',
                ll.school_district as 'School District',
                
                -- Property Details
                ld.bathrooms_full as 'Full Bathrooms',
                ld.bathrooms_half as 'Half Bathrooms',
                ld.above_grade_finished_area as 'Above Grade Area',
                ld.below_grade_finished_area as 'Below Grade Area',
                ld.building_area_total as 'Total Building Area',
                ld.lot_size_acres as 'Lot Size (Acres)',
                ld.lot_size_square_feet as 'Lot Size (Sq Ft)',
                ld.stories_total as 'Stories',
                ld.rooms_total as 'Total Rooms',
                ld.structure_type as 'Structure Type',
                ld.architectural_style as 'Architectural Style',
                ld.fireplace_yn as 'Has Fireplace',
                ld.fireplaces_total as 'Number of Fireplaces',
                ld.garage_yn as 'Has Garage',
                ld.garage_spaces as 'Garage Spaces',
                ld.parking_total as 'Total Parking',
                ld.public_remarks as 'Public Remarks',
                
                -- Financial Information
                lf.tax_annual_amount as 'Annual Taxes',
                lf.tax_year as 'Tax Year',
                lf.association_yn as 'Has HOA',
                lf.association_name as 'HOA Name',
                lf.association_fee as 'HOA Fee',
                lf.association_fee_frequency as 'HOA Fee Frequency',
                
                -- Features
                lft.waterfront_yn as 'Waterfront',
                lft.pool_private_yn as 'Private Pool',
                lft.view_yn as 'Has View',
                lft.virtual_tour_url_unbranded as 'Virtual Tour URL',
                
                -- Agent Information
                l.list_agent_mls_id as 'Listing Agent MLS ID',
                l.buyer_agent_mls_id as 'Buyer Agent MLS ID',
                l.list_office_mls_id as 'Listing Office MLS ID',
                l.buyer_office_mls_id as 'Buyer Office MLS ID',
                
                -- Dates
                l.creation_timestamp as 'Created Date',
                l.modification_timestamp as 'Last Modified',
                l.status_change_timestamp as 'Status Changed',
                l.close_date as 'Close Date',
                l.listing_contract_date as 'Listing Contract Date',
                
                -- Additional
                l.photos_count as 'Photos Count',
                l.open_house_yn as 'Has Open House'
        ...
        // Generate filename
        $count = count($results);
        $filename = 'mls-listings-' . $count . '-records-' . date('Y-m-d-H-i-s') . '.csv';

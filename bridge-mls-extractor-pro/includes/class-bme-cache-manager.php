...
class BME_Cache_Manager {
    ...
    private function delete_pattern($pattern) {
        ...
        // In production, you might want to use Redis or Memcached for better pattern support
        
        if (function_exists('wp_cache_flush_group')) {
            // If using object cache with groups, flush the entire group
            wp_cache_flush_group($this->cache_group);
        } else {
            // Fallback: flush all cache (not ideal but necessary)
            wp_cache_flush();
        }
    }
